...
class Vocab(object):
        ...
        with open(embedding_path, 'r') as fin:
            for line in fin:
                ...
                try:
                    ...
                    continue
                if token not in self.token2id or len(contents) == 2:
                    continue
                if flag and len(contents) - 1 != self.embed_dim:
                    continue
                trained_embeddings[token] = list(map(float, contents[1:]))
                matched_num += 1
                if self.embed_dim is None:
                    self.embed_dim = len(contents) - 1
                elif self.embed_dim != len(contents) - 1:
                    print 'Original dim {} is not equal to dim {} of the pretrained embedding'.format(
                        self.embed_dim, len(contents) - 1)
                    print 'Changed the dim'
                    self.embed_dim = len(contents) - 1
                    flag = True
        filtered_tokens = trained_embeddings.keys()
        # rebuild the token x id map
        original_tokens = self.token2id.keys()
        self.token2id = {}
        self.id2token = {}
        for token in self.initial_tokens + original_tokens:
            self.add(token, cnt=0)
        for token in filtered_tokens:
            self.add(token, cnt=0)
        # load embeddings
        self.embeddings = np.zeros([self.size(), self.embed_dim])
        print 'Matched {} / {} words'.format(matched_num, self.size())
        for token in self.token2id.keys():
            if token in trained_embeddings:
                self.embeddings[self.get_id(token)] = trained_embeddings[token]

        self.embeddings = normalize(self.embeddings, axis=1, norm='l2')
    ...
    def convert_to_ids(self, tokens):
        ...
        Returns:
            a list of ids
        """
        vec = [self.get_id(label.lower()) for label in tokens]
        return vec
